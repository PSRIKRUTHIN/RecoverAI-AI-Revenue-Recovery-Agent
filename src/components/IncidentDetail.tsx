...
interface IncidentDetailProps {
  ...
  // Opens the simulated RecoverAI checkout
  onOpenFastLink: (incident: RecoveryIncident) => void;
...
export const IncidentDetail: React.FC<IncidentDetailProps> = ({
  ...
  const getDefaultPtpNotes = (
    currentIncident: RecoveryIncident
  ) => {
    // B2B receivables need business-process-specific
    // notes instead of consumer payment notes.
    if (
      currentIncident.channel === 'b2b_invoice'
    ) {
      return 'Customer committed to settlement after AP procurement approval and completion of the PO matching process. Soft freeze applied.';
    }

    // Insufficient funds is treated as a temporary
    // liquidity/funds-availability situation.
    if (
      currentIncident.errorCode === 'insufficient_funds'
    ) {
      return 'Customer confirmed funds availability window. Soft freeze applied.';
    }

    // Generic fallback for other incidents.
    return 'Customer confirmed settlement window. Soft freeze applied.';
  };
  ...
  const [ptpNotes, setPtpNotes] = useState(
    getDefaultPtpNotes(incident)
  );

  // =========================================================
  // RESET PTP FORM WHEN INCIDENT CHANGES
  // =========================================================

  useEffect(() => {
    setPtpDate(getDefaultPtpDate());
    setPtpAmount(incident.amount);
    setPtpNotes(getDefaultPtpNotes(incident));
  }, [
    incident.id,
    incident.amount,
    incident.channel,
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-4">
            ...
            <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3 flex items-center justify-between">
              ...
              <span className="text-[11px] font-mono text-amber-700 font-semibold">
                Attempt {incident.attempts} of{' '}
                {incident.maxAttemptsAllowed}
              </span>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div>
                ...
                <span className="text-slate-800 font-medium">
                  {incident.metadata?.gatewayName ||
                    'Stripe API / Card Scheme'}
                </span>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-mono text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-200 font-medium">
                  {incident.diagnosis?.aiModel ||
                    'Groq — GPT-OSS-20B'}
                </span>

                <button
                  onClick={() =>
                    onDiagnoseWithAi(incident)
                  }
          ...
          <div className="bg-white border border-slate-200/90 rounded-xl p-4 shadow-xs">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
              ...
              <button
                id="btn-action-fastlink"
                onClick={() =>
                  onOpenFastLink(incident)
                }
                disabled={
                  incident.status === 'recovered' ||
                  incident.status === 'stopped_compliant'
                }
                className="p-2.5 rounded-lg text-xs font-semibold bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 flex flex-col items-center justify-center gap-1 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-xs"
              >
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 max-w-md w-full shadow-2xl text-slate-900">
            ...
            >
              ...
              <div className="p-2.5 rounded-lg bg-sky-50 border border-sky-200 text-sky-800 text-[11px]">
                ...
                {incident.channel ===
                'b2b_invoice'
                  ? 'Freezes dunning pings until the promised settlement date, preserves the active commercial relationship, and prevents additional recovery attempts during the commitment window.'
                  : 'Freezes dunning pings, preserves active subscriptions, and schedules the next bounded recovery action within the permitted recovery window.'}
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 max-w-md w-full shadow-2xl text-slate-900">
            ...
            >
              ...
              <div className="p-2.5 rounded-lg bg-rose-50 border border-rose-200 text-rose-800 text-[11px]">
                ...
                Permanently stops automated contact and
                token presentation for this incident.
                Protects your merchant account from
                penalties.
